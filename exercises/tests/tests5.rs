...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: We assume the caller has upheld the contract specified in the
    // safety documentation, ensuring that `address` is a valid, mutable
    // reference to a `u32`.
    unsafe {
        let ptr = address as *mut u32; // 将 usize 转换为 *mut u32
        *ptr = 0xAABBCCDD; // 解引用指针并赋值
    }
}
